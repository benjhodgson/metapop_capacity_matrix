...
# Define maximum movement increases (x increase)
movement_abilities <- c(1.1, 1.5, 6, 11, 51)
...
# Slope constants
slopes <- data.frame(
  m = c("linear", "linear", "linear", "linear", "linear", 
        "concave", "concave", "concave", "concave", "concave", 
        "convex", "convex", "convex", "convex", "concave"),
  m_a = c(1.1, 1.5, 6, 11, 51, 
          1.1, 1.5, 6, 11, 51, 
          1.1, 1.5, 6, 11, 51),
  s1 = c(0.1, 0.5, 5, 10, 50,  
         0.1, 0.5, 5, 10, 50, 
         4.54e-6, 2.27e-5, 2.27e-4, 4.53e-4, 0.00227),
  s2 = c(NA, NA, NA, NA, NA,
         NA, NA, NA, NA, NA,
         0.99999546, 0, 0, 0, 0.99773),
  a = c(1, 1, 1, 1, 1,
        0.1, 0.5, 5, 10, 50, 
        0, 1, 1, 1, 0)
)
...
for (j in 1:nrow(movement_combos)) {
  ...
  rep_function <- function(movement, movement_ability) {
    ...
    repeat {
      ...
      landscape <- nlm_randomcluster(ncol = x_extent/resolution, nrow = y_extent/resolution, 
      ...
      landscape_df$patch <- raster::extract(clumps, landscape_df)
      landscape_df <- as.data.frame(landscape_df)
    ...
    # view landscape
    ggplot(landscape_df, aes(x = x, y = y, fill = factor(patch))) +
      geom_tile() +
      theme_minimal() +
      theme(
        axis.text = element_blank(),   # Hide axis text
    ...
    dispersal_intense <- 8 # the mean dispersal distance under yield = 1
