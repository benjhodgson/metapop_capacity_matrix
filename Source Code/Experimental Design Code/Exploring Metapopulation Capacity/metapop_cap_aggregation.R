...
for (j in 1:nrow(movement_combos)) {
  ...
  rep_function <- function(movement, movement_ability) {
    ...
    landscape_config <- 0.01 # level of patch aggregation
  ...
  num_reps <- 20 # specify the number of repeats
...
result_la <- result_final_complete

saveRDS(result_la, "Results/Results_Folder/Exploring Metapopulation Capacity/Metapopulation Capacity Aggregation/result_la")
